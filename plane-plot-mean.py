...
data=np.genfromtxt(f'avg-radial-cut-0deg-{variable}.txt',skip_header=15)
...
for degree in degrees:
    ...
    data=np.genfromtxt(f'avg-radial-cut-{degree}deg-{variable}.txt',skip_header=15)
    df=pd.DataFrame({'x':data[:,0],'y':data[:,1],'z':data[:,2],'var':data[:,3]})
    ...
    print('Saving plot')
    plt.tight_layout()
    plt.savefig(os.path.join(f'images/planes/{variable}',f'Znearest-{degree}deg_{variable}.png'),dpi=600)
    plt.close()
    ...
    print('Saving plot')
    plt.tight_layout()
    plt.savefig(os.path.join(f'images/planes/{variable}',f'Zcombined-{degree}deg_{variable}.png'),dpi=600)
    plt.close()
    ...
    print('Saving plot')
    plt.tight_layout()
    plt.savefig(os.path.join(f'images/planes/{variable}',f'Z-delta-{degree}deg_{variable}.png'),dpi=600)
    plt.close()
    ...
    print('Saving plot')
    plt.tight_layout()
    plt.savefig(os.path.join(f'images/planes/{variable}',f'Z-epsilon-{degree}deg_{variable}.png'),dpi=600)
    plt.close()
    print('\n')
...
print('Saving plot')
plt.tight_layout()
plt.savefig(os.path.join(f'images/planes/{variable}',f'Zmean-{degree}deg_{variable}.png'),dpi=600)
plt.close()

Z_filled = np.nan_to_num(Z_mean, nan=-1e6)
exporter = VTKExporter(Xg, Yg, Z_filled, variable)
exporter.SaveVTI(f'{variable}_mean-plane')
